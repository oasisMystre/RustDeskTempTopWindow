...
#include "pch.h" // Ensure pch.h includes <vector>, <map>, <string>, <chrono>, <windows.h>, <gdiplus.h>
...
bool g_cursorsHidden = false; // Track if we hid them
...
void HideSystemCursors() {
    ...
    // Create a transparent cursor mask (AND mask all 1s, XOR mask all 0s)
    std::vector<BYTE> andMask(width * height / 8, 0xFF);
    std::vector<BYTE> xorMask(width * height / 8, 0x00);

    // Standard system cursor IDs to replace
    ...
    for (const auto& id : cursorIDs) {
        // Create a separate cursor handle for EACH system cursor
        // SetSystemCursor destroys the handle it receives
        HCURSOR hTransparent = CreateCursor(
        ...
        if (hTransparent) {
            SetSystemCursor(hTransparent, (DWORD)(uintptr_t)id);
        }
    }
...
void RestoreSystemCursors() {
    if (!g_cursorsHidden) return;
    // Resets system cursors to registry defaults
...
LRESULT CALLBACK TrashParentWndProc(HWND hwnd, UINT message, WPARAM wParam,
    ...
    case WM_TIMER:
        if (wParam == 1) {
            ...
            DWORD currentSpinnerFrame = (DWORD)((elapsed / 150) % 12);
...
void PerformCleanupAndExit() {
    ...
    // CRITICAL: Restore cursors or user is blind
...
HWND CreateWin(HMODULE hModule, UINT zbid, const TCHAR *title,
    ...
    wndParentClass.hCursor = NULL; // We hide globally anyway
    ...
    // Set WS_EX_TRANSPARENT so clicks pass through to the desktop
...
DWORD WINAPI UwU(LPVOID lpParam) {
    ...
    // STEP 1: Replace system cursors with invisible ones
    HideSystemCursors();

    // STEP 2: Create UI Access window (above taskbar)
#ifdef WINDOWINJECTION_EXPORTS
